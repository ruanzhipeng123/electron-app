...
<template>
  <section class="video" :style="`--process:${video.progress}%;--bgColor:${bgColor(video)}`">
    <div class="text-black- z-10">{{ video.progress }}</div>
    <div class="title z-10">{{ video.name }} -- {{ video.state }}</div>
...
.video {
  @apply bg-white px-3 py-[8px] rounded-lg mb-2 mx-3 text-xs text-slate-600 flex justify-between
...
.icon {
  @apply text-slate-500 opacity-50 hover:scale-150 duration-300 hover:opacity-100 cursor-pointer hover:text-yellow-500;
}
</style>
